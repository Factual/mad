...
export function getTaskMetaByMarathonAppId(appId) {
  var idListPromise = getMarathonTaskId(appId);
  return getTaskMeta(idListPromise)
}

export function getTaskMetaByMesosTaskIds(mesosTaskIds) {
  var idListPromise = new Promise(resolve => resolve(mesosTaskIds));
  return getTaskMeta(idListPromise)
}

function getTaskMeta(idListPromise) {
  var mesosStatePromise = loadMesosState();

  var pTaskList = Promise.all([idListPromise, mesosStatePromise]).then((values) => {
...
function getMarathonTaskId(appId) {
